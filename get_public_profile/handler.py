...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if not items:
            ...
            providers_table_name = os.environ.get(
                "PROVIDERS_TABLE", "ChatBooking-Providers"
            )
            ...
            try:
                ...
                logger.info(
                    f"Found provider {provider_id} for slug {slug} in tenant {tenant_id}"
                )
        ...
        services = []
        try:
            services_table_name = os.environ.get(
                "SERVICES_TABLE", "ChatBooking-Services"
            )
            ...
            services_response = services_table.scan(
                ...
                ExpressionAttributeValues={":tid": tenant_id, ":active": True},
            )
        ...
        try:
            ...
            for prov in raw_providers:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse provider metadata for {prov.get('providerId')}",
                        error=str(e),
                    )
                ...
                providers.append(
                    {
                        ...
                        "serviceIds": prov.get(
                            "services", []
                        ),  # Correct field name in DB is services
        ...
        public_profile = {
            ...
            or settings.get(
                "primaryServiceId"
            ),  # This usually stays at root or needs check
            "services": services,
            ...
            "preselectedProviderId": (
                target_provider_id if is_provider_profile else None
            ),
        }
        ...
        if is_provider_profile:
            ...
            target_provider = next(
                (p for p in providers if p["providerId"] == target_provider_id), None
            )

            if target_provider:
                public_profile["name"] = target_provider["name"]
                public_profile["bio"] = (
                    target_provider.get("bio") or public_profile["bio"]
                )
                public_profile["photoUrl"] = (
                    target_provider.get("photoUrl") or public_profile["photoUrl"]
                )
                public_profile["specializations"] = (
