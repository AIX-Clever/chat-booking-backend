...
def test_profile_baker_provider_event():
    ...
    mock_s3.get_object.return_value = {
        'Body': MagicMock(read=lambda: b'<html><head><title>Original</title></head><body>{profile_data}</body></html>')
    }
    ...
    def get_table(name):
        if name == 'Tenants': return mock_tenants_table
        if name == 'Services': return mock_services_table
        if name == 'Providers': return mock_providers_table
        return MagicMock()
