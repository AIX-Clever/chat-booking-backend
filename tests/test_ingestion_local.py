    ...
    def test_schema_intialization_and_dimensions(self, mock_boto, mock_execute):
        ...
        def check_bedrock_call(*args, **kwargs):
            ...
            if body.get("dimensions") != 1024:
                raise ValueError(
                    f"CRITICAL: Dimensions set to {body.get('dimensions')}, expected 1024!"
                )
        ...
        from knowledge_base.ingestion_handler import lambda_handler
        from shared.ai_handler import AIHandler
        from shared.infrastructure.vector_repository import VectorRepository

        # Initialize Repo & Handler
        # Note: In real Lambda these are global, here we institute them
        with patch("shared.ai_handler.boto3.client") as mock_ai_boto:
            mock_ai_boto.return_value = mock_bedrock
        ...
        create_calls = [
            call for call in mock_execute.call_args_list if "CREATE TABLE" in call[0][0]
        ]
        self.assertTrue(
            len(create_calls) > 0, "ensure_schema did not try to CREATE TABLE"
        )
        ...
        try:
            ...
            print(
                "✅ Unit Test: AIHandler.get_embedding() successfully called with 1024 dimensions."
            )
