...
class StateTransition:
    ...
    def can_transition(self, conversation: Conversation) -> tuple[bool, Optional[str]]:
        ...
        if conversation.state != self.from_state:
            return (
                False,
                f"Cannot transition from {conversation.state.value} to {self.to_state.value}",
            )
...
class ChatFSM:
    ...
    TRANSITIONS = {
        ...
        ConversationState.SERVICE_PENDING: [
            ...
            StateTransition(
                ConversationState.SERVICE_PENDING, ConversationState.PROVIDER_PENDING
            ),
        ],
        ConversationState.SERVICE_SELECTED: [
            StateTransition(
        ...
        ConversationState.BOOKING_CONFIRMED: [
            StateTransition(
                ConversationState.BOOKING_CONFIRMED, ConversationState.SERVICE_PENDING
            )
        ],
    }
    ...
    @classmethod
    def can_transition(
        cls, conversation: Conversation, to_state: ConversationState
        ...
        return (
            False,
            f"No valid transition from {current_state.value} to {to_state.value}",
        )

    @classmethod
    def get_next_states(
        cls, current_state: ConversationState
    ...
    def get_required_fields(
        cls, from_state: ConversationState, to_state: ConversationState
    ) -> list[str]:
        """
    ...
    def validate_context(
        cls, state: ConversationState, context: Dict[str, Any]
    ) -> tuple[bool, Optional[str]]:
        """
        ...
        required_by_state = {
            ...
            ConversationState.PROVIDER_SELECTED: [
                "providerId"
            ],  # serviceId might still be missing if inferred later
            ConversationState.PROVIDER_SELECTED: ["providerId"],
            ConversationState.SLOT_PENDING: [
                "providerId"
            ],  # Service ID might be determined by slot or provider
            ConversationState.CONFIRM_PENDING: [
                "serviceId",
                "providerId",
                "selectedSlot",
        ...
        if missing:
            return (
                False,
                f"Missing required fields for {state.value}: {', '.join(missing)}",
            )
