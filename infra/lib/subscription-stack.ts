...
export class SubscriptionStack extends cdk.Stack {
    ...
    constructor(scope: Construct, id: string, props: SubscriptionStackProps) {
        ...
        // Pre-bind resources for QA to bypass EarlyValidation hook issues with cross-stack references in the OLD account
        // For PROD in the NEW account, we prefer standard props to allow clean stack dependencies
        ...
        const userPool = (props.envName === 'qa')
            ? cognito.UserPool.fromUserPoolId(this, 'UserPoolPreBound', 'us-east-2_BJ7JY5CCl')
            : props.userPool;
