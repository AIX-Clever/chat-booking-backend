...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Handles Microsoft OAuth flow via Function URL.
    Routes:
    - GET /authorize: Redirects to Microsoft consent screen
    ...
    auth_service = MicrosoftAuthService(client_id, client_secret, redirect_uri)
...
def handle_authorize(params: dict, auth_service: MicrosoftAuthService) -> Dict[str, Any]:
...
def handle_callback(params: dict, auth_service: MicrosoftAuthService, repo: DynamoDBProviderIntegrationRepository) -> Dict[str, Any]:
    code = params.get('code')
    state = params.get('state')
    error = params.get('error')
    
    if error:
        return _response(400, f"Microsoft Auth Error: {error}")
    ...
    try:
        ...
        # Save tokens
        repo.save_microsoft_creds(tenant_id, provider_id, tokens)
        
        return _response(200, "Successfully connected Outlook Calendar! You can close this window.")
