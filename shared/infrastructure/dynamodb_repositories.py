...
class DynamoDBApiKeyRepository(IApiKeyRepository):
    ...
    def _item_to_entity(self, item: dict) -> ApiKey:
        return ApiKey(
            ...
            created_at=datetime.fromisoformat(item["createdAt"]),
            last_used_at=(
                datetime.fromisoformat(item["lastUsedAt"])
                if item.get("lastUsedAt")
                else None
            ),
        )
    ...
    def get_by_id(self, tenant_id: TenantId, booking_id: str) -> Optional[Booking]:
        ...
        try:
            response = self.table.scan(
                ...
                & Attr("tenantId").eq(str(tenant_id))
            )
    ...
    def list_by_provider(
        ...
        try:
            ...
            response = self.table.query(
                ...
                KeyConditionExpression=Key("tenantId_providerId").eq(
                    f"{tenant_id}_{provider_id}"
                )
    ...
    def list_by_customer_email(
        ...
        try:
            ...
            customer_id = hashlib.md5(
                customer_email.lower().strip().encode()
            ).hexdigest()

            gsi_pk = f"{tenant_id}#{customer_id}"
            response = self.table.query(
                IndexName="GSI1", KeyConditionExpression=Key("GSI1PK").eq(gsi_pk)
            )
...
class DynamoDBConversationRepository(IConversationRepository):
    ...
    def __init__(self, table_name: Optional[str] = None):
        ...
        self.table = self.dynamodb.Table(
            table_name
            or os.environ.get("CONVERSATIONS_TABLE", "ChatBooking-Conversations")
        )
    ...
    def _item_to_entity(self, item: dict) -> Conversation:
        return Conversation(
            ...
            slot_start=(
                datetime.fromisoformat(item["slotStart"])
                if item.get("slotStart")
                else None
            ),
            slot_end=(
                datetime.fromisoformat(item["slotEnd"]) if item.get("slotEnd") else None
            ),
            ...
            created_at=(
                ...
                else datetime.fromisoformat(item["updatedAt"])
            ),
...
class DynamoDBFAQRepository(IFAQRepository):
    ...
    def delete(self, tenant_id: TenantId, faq_id: str) -> None:
        self.table.delete_item(Key={"tenantId": str(tenant_id), "faqId": faq_id})
