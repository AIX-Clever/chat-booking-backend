...
class DynamoDBAvailabilityRepository(IAvailabilityRepository):
    ...
    def __init__(self, table_name: Optional[str] = None):
        ...
        self.table = self.dynamodb.Table(
            table_name
            or os.environ.get("AVAILABILITY_TABLE", "ChatBooking-Availability")
        )
    ...
    def get_provider_exceptions(
        ...
        try:
            ...
            response = self.table.get_item(
                Key={"tenantId_providerId": pk, "dayOfWeek": "EXCEPTIONS"}
            )
    ...
    def _item_to_entity(self, item: dict) -> ProviderAvailability:
        ...
        breaks = [
            TimeRange(br["startTime"], br["endTime"]) for br in item.get("breaks", [])
        ]
