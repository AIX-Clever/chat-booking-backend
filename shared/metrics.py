...
class MetricsService:
    ...
    def __init__(self, table_name: Optional[str] = None):
        ...
        self.table_name = table_name or os.environ.get('TENANT_USAGE_TABLE', 'ChatBooking-TenantUsage')
    ...
    def _atomic_increment(
        ...
        update_expression = 'ADD #attr :inc SET #ttl = if_not_exists(#ttl, :ttl), #updatedAt = :now'
        expression_names = {
            '#attr': attribute, 
            '#ttl': 'ttl',
            '#updatedAt': 'updatedAt'
    ...
    def increment_message(self, tenant_id: str, is_ai_response: bool = False) -> None:
        ...
        if is_ai_response:
            self._atomic_increment(tenant_id, f"MONTH#{periods['month']}", 'aiResponses')
    
    def increment_tokens(self, tenant_id: str, token_count: int) -> None:
        """Track AI token usage"""
        periods = self._get_periods()
        self._atomic_increment(tenant_id, f"MONTH#{periods['month']}", 'tokensIA', count=token_count)
    ...
    def increment_conversation_completed(self, tenant_id: str) -> None:
        ...
        periods = self._get_periods()
        self._atomic_increment(tenant_id, f"MONTH#{periods['month']}", 'conversionsChat')
    
    def update_booking_status(self, tenant_id: str, old_status: str, new_status: str) -> None:
        ...
        if old_status:
            self._atomic_increment(
                tenant_id, 
                f"STATUS#{old_status}#{periods['month']}", 
                'count',
                count=-1
            )
        ...
        self._atomic_increment(
            tenant_id, 
            f"STATUS#{new_status}#{periods['month']}", 
            'count'
        )
    ...
    def get_dashboard_metrics(self, tenant_id: str) -> Dict[str, Any]:
        ...
        for item in items:
            ...
            if sk.startswith(f'MONTH#{current_month}'):
                ...
                result['summary']['conversionsChat'] = int(item.get('conversionsChat', 0))
        ...
        if total_bookings > 0:
            result['summary']['conversionRate'] = round(
                (result['summary']['conversionsChat'] / result['summary']['messages'] * 100) 
                if result['summary']['messages'] > 0 else 0, 
                1
            )
            result['summary']['autoAttendanceRate'] = round(
                (result['summary']['aiResponses'] / result['summary']['messages'] * 100) 
                if result['summary']['messages'] > 0 else 0, 
                1
            )
        else:
