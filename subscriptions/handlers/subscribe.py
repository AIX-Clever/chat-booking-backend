    ...
    try:
        ...
        if payment_method == 'fintoc':
            ...
            try:
                ...
                # Note: Fintoc flow starts with Widget, then we get a Link Token.
                # However, for 'subscribe', maybe we want to return the widget token?
                # Actually, Fintoc Widget is client-side. The user connects their account, 
                # gets a Link Token, and THEN we should probably create the subscription/mandate?
                # OR, if we are just starting the process, we return a token to open the widget.
                
                # Let's assume this mutation initializes the flow.
                # We return a 'widget_token' as 'initPoint' (or specialized field)
                ...
                raise Exception(f"Failed to initialize Fintoc: {str(e)}")
